...
class DefaultTemplatesBuilder
{
    public function buildBlogLink()
    {
        $db = JFactory::getDbo();
        ...
        if (count($rows) > 0)
            return 'index.php?option=com_content&view=category&layout=blog&id=' . $rows[0]->id;
        return '';
    }

    public function buildPostLink()
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('*')
        ...
        if ($ret !== null)
            return 'index.php?option=com_content&view=article&id=' . $ret->id;
        return '';
    }

    public function buildProductsLink()
    {
        if (!JComponentHelper::getComponent('com_virtuemart', true)->enabled)
            return '';

        $tableList = JFactory::getDbo()->getTableList();
        $vmCatsTable = JFactory::getConfig()->get('dbprefix') . 'virtuemart_categories';
        if (!in_array($vmCatsTable, $tableList)) {
            return '';
        }
        ...
        if (count($rows) > 0)
            return 'index.php?option=com_virtuemart&view=category&virtuemart_manufacturer_id=0&categorylayout=0&showcategory=1&showproducts=1&productsublayout=0&virtuemart_category_id=' . $rows[0]->virtuemart_category_id;
        return '';
    }

    public function buildProductOverviewLink()
    {
        if (!JComponentHelper::getComponent('com_virtuemart', true)->enabled)
            return '';

        $tableList = JFactory::getDbo()->getTableList();
        $vmCatsTable = JFactory::getConfig()->get('dbprefix') . 'virtuemart_products';
        if (!in_array($vmCatsTable, $tableList)) {
            return '';
        }
        ...
        if (count($rows) > 0)
            return 'index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $rows[0]->virtuemart_product_id;
        return '';
    }

    public function buildShoppingCartLink()
    {
        if (!JComponentHelper::getComponent('com_virtuemart', true)->enabled)
            return '';
        ...
        return 'index.php?option=com_virtuemart&view=cart';
    }
...
EOF;
                ...
                $app->redirect(dirname(dirname((JURI::current()))) . '/templates/' . $themeName . '/app/tmpl/_warning.php');
            }
        } else {
            ob_start();
            ?>
            <script>if ('undefined' != typeof jQuery) document._jQuery = jQuery;</script>
            <script src="<?php echo dirname(dirname((JURI::current()))) . '/templates/' . $themeName . '/jquery.js' ?>" type="text/javascript"></script>
            ...
            <script>
                ...
                jQuery(function ($) {
                    var btnSelector = '#<?php echo $config['parameterObject']->id; ?>',
                        appPath = '<?php echo dirname(dirname((JURI::current()))) . '/templates/' . $themeName . '/app'; ?>',
                        params = {};
    ...
    public static function checkPermissions() {
        ...
        if ('' !== $permissionsResult) {
            if (!file_exists($tmpPath)){
                $permissionsResult .= "<br />";
                $permissionsResult .= "<strong class='tmp-path'>Path to Temp Folder is wrong</strong>";
            }
            $content = <<<EOF
<style>
    .msgbox button { width: 60px; height: 25px; font-size: 13px; margin-left: 5px; }
...
EOL;
    ...
    public static function buildPreview()
    {
        ...
        foreach ($iterator as $fileInfo) {
            ...
            $fname      = $parts[0];
            if (preg_match('/^plugins\//', $path) && $ext == 'zip') {
                //supported plugins
                $pluginsList = array('button', 'content', 'exportimport');
                if (!in_array($fname, $pluginsList)) {
                    Helper::deleteFile($themeDir . '/' . $path);
                    continue;
                }
                if (version_compare(JVERSION, '3.0', '<')) {
                    jimport('joomla.filesystem.archive');
                    $ret = JArchive::extract($themeDir . '/' . $path, $editorDir . '/plugins');
                    if ($ret === false) {
                        // to do
                    }
                } else {
                    ...
                    try {
                        JArchive::extract($themeDir . '/' . $path, $editorDir . '/plugins');
                    } catch (Exception $e) {
                        // to do
                    }
                }
    ...
    {
        $startFiles = array();
        $base = dirname(dirname((JURI::current()))) . '/';
        $themeObject = Config::getStyleObject();
        $themeName    = $themeObject->template;
        $themeDir     = JPATH_SITE . '/templates/' . $themeName;
        ...
        $startFiles['manifest'] = $version ? $base . 'templates/manifests/manifest.php?ver=' . $version : '';

        $hash = md5(round(microtime(true)));
        $startFiles['project'] = $base . 'templates/' . $themeName . '/app/start/data.php?version=' . $hash;
        $startFiles['templates'] = $base . 'templates/' . $themeName . '/app/start/templates.php?version=' . $hash . '&postId=' . JRequest::getVar('postId', '');;
        $startFiles['auth'] = $base . 'templates/' . $themeName . '/app/start/auth.php?uid=' . JFactory::getUser()->id . '&version=' . $hash;
    ...
    public static function getConfigObject()
    {
        ...
        $infoData['doImportData'] = false;
        $registry = new JRegistry();
        $registry->loadString($themeObject->params);
        $params =  $registry->toArray();
        if ((!isset($params['contentImported']) || $params['contentImported'] !== '1') && file_exists($themeDir . '/data/content.json')) {
            $infoData['doImportData'] = true;

            $params['contentImported'] = '1';
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select('params')->from('#__template_styles')->where('id=' . $query->escape($themeObject->id));
            $db->setQuery($query);
            $registry = new JRegistry();
            $registry->loadString($db->loadResult());
            $parameters = $registry->toArray();
            ...
            $query->update('#__template_styles')->set(
                $db->quoteName('params') . '=' .
                $db->quote($registry->toString())
            )->where('id=' . $query->escape($themeObject->id));
            $db->setQuery($query);
            $db->query();
        }

        $infoData['isThemeActive'] = 1 == $themeObject->home ? true : false;
        $infoData['exportImportPlgInstalled'] = Config::isThemlerExportImportPluginInstalled() ? true : false;
        ...
        $infoData['contentManagerPage'] =  $base . 'administrator/index.php?option=com_content';
        ...
        Helper::writeFile($themeDir . '/app/app.lock', $time);

        return json_encode(array(
            'upage_editor'  => $base . 'administrator/index.php?option=com_content&view=article&layout=edit&editor=1',
    ...
    public static function getDuplicateTemplatesConstructors($dir)
    {
        ...
        $new = array();
        $i = 0;
        foreach ($constructors as $key => $props) {
            $slice = array_slice($resultTemplatesList, $i, 1);
            $info = array_pop($slice);
            $i++;
            if ($info['virtuemart'] == 'yes' && !JComponentHelper::getComponent('com_virtuemart', true)->enabled)
                continue;
            $new[$props['constructor']] = $props['canDuplicate'];
        }
        return $new;
     ...
     * @return bool
     */
    public static function isPageCheckItem($params)
    {
        $itemParams = $params->toArray();
        ...
        if (isset($itemParams['show_category'])
            && '0' !== $itemParams['show_category']
            && '' !== $itemParams['show_category'])
            return false;
        if (isset($itemParams['show_create_date'])
            && '0' !== $itemParams['show_create_date']
            && '' !== $itemParams['show_create_date'])
            return false;
        if (isset($itemParams['show_modify_date'])
            && '0' !== $itemParams['show_modify_date']
            && '' !== $itemParams['show_modify_date'])
            return false;
        if (isset($itemParams['show_publish_date'])
            && '0' !== $itemParams['show_publish_date']
            && '' !== $itemParams['show_publish_date'])
            return false;
        if (isset($itemParams['show_author'])
            && '0' !== $itemParams['show_author']
            && '' !== $itemParams['show_author'])
            return false;
        if (isset($itemParams['show_print_icon'])
            && '0' !== $itemParams['show_print_icon']
            && '' !== $itemParams['show_print_icon'])
            return false;
        if (isset($itemParams['show_email_icon'])
            && '0' !== $itemParams['show_email_icon']
            && '' !== $itemParams['show_email_icon'])
            return false;
    ...
    {
        $tmplBuilder = new DefaultTemplatesBuilder();

        $themeName = Config::getStyleObject()->template;
        $previewParams = 'template=' . $themeName . '&is_preview=on';
        $current = dirname(JURI::current()) . '/';
        ...
        if (file_exists($pathToManifest)) {
            ...
            if (version_compare(JVERSION, '3.0', '>')) {
                $site->loadLanguage($language);
                JFactory::$language = $site->getLanguage();
            } else {
            ...
            if (null !== $defaultMenu) {
                // home link
                $parts = explode('-', $language->getTag());
                $defaultHomeLink = $defaultMenu->link . (isset($defaultMenu->id) ? '&Itemid=' . $defaultMenu->id : '') .
            ...
            // menus with 'hmenu' position process first
            $menuModules = array_merge($modulesHmenuPosition, $menuModules);
                    ...
                    if (isset($item->query['option'])) {
                        ...
                        if (isset($item->query['view'])) {
                            ...
                            if (isset($item->query['layout'])) {
                                $value .= '@' . $item->query['layout'];
                            }
                            if ('category' === $view && isset($item->query['categorylayout']) && $item->query['categorylayout'] != 'categories') {
                                $value .= '@products';
                            }
                            if (isset($item->query['id']) && 'article' === $view) {
                                $value .= Config::isPageCheckItem($item->params) ? '@is_page' : '';
                            }
                        }
                        ...
                        if ($item->home === '1') {
                            $homeLinks[$item->language] = $link . $lang;
                            continue;
                        }
                        if (!array_key_exists($value, $allTemplatesList)) {
            ...
            $homeLink = '';
            if (!Config::isLanguageFilterEnabled() && '' !== $defaultHomeLink) {
                $homeLink = $defaultHomeLink;
            } else {
                $mainLanguage = $site->input->cookie->getString(JApplication::getHash('language'));
                if (!$mainLanguage) {
                    $mainLanguage = JComponentHelper::getParams('com_languages')->get('site', 'en-GB');
                }
                if (array_key_exists($mainLanguage, $homeLinks)) {
                    $homeLink = $homeLinks[$mainLanguage];
                } else {
                    if (array_key_exists('*', $homeLinks))
                        $homeLink = $homeLinks['*'];
                }
            }
            ...
            foreach($templatesInfo as $templateName => $item) {
                if ('error404' === $item['kind']) {
                    $resultTemplatesList[$templateName] = $root . 'index.php?option=com_template_error&' .
                ...
                if (isset($item['buildDefaultFunc']) && ($ret  = $tmplBuilder->{$item['buildDefaultFunc']}()) != '') {
                    $url  = $root . $ret . '&' . $previewParams . '&file_template_name=' . $item['fileName'];
                    if (strpos($item['action'], 'is_page') !== false)
                        $url .= '&includePageTemplate=1';
                }
            ...
            foreach($templatesInfo as $templateName => $item) {
                ...
                foreach($allTemplatesList as $info) {
                    if ('true' === $info['selected'])
                        continue;
                    $resultTemplatesList[$templateName] = $root . ($info['url'] ? $info['url'] . '&' : '?') .
                        $previewParams . '&file_template_name=' . $item['fileName'];
                    break;
                }
            }
        ...
        if ('' !== $postId) {
            ...
            if (null !== $Itemid) {
                ...
                $postUrl = $root . ContentHelperRoute::getArticleRoute($postId);
                $postUrl = preg_replace('/&Itemid=(\d+)/', '&Itemid=' . $Itemid, $postUrl);
                $parts = explode('-', JComponentHelper::getParams('com_languages')->get('site', 'en-GB'));
                if ($parts > 1)
                    $postUrl .= '&lang=' . array_shift($parts);
                $postUrl .= '&template=' . $themeName . '&is_preview=on';
            }
        }

        $homeLink = $postUrl ? $postUrl : ($root . ($homeLink ? $homeLink . '&' : '?') . $previewParams);
        return array('home' => $homeLink, 'templates' => $resultTemplatesList);
    }

    /**
     * @return bool
     */
    public static function isLanguageFilterEnabled()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where('element = ' . $db->quote('languagefilter'))
    ...
    public static function isThemlerExportImportPluginInstalled()
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true)
            ->select('*')
